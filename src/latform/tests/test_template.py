...
def test_header_stable_under_reformat(tmp_path):
    """The header goes through the output formatter, so reformatting a
    generated lattice file is a no-op."""
    spec = _header_spec(tmp_path)
    res = instantiate(
        spec, base_dir=tmp_path, options=default_options, instances_path="instances.yaml"
    )["c1"]
    text = res["c1.lat.bmad"]
